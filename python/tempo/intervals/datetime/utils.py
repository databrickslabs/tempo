...
def infer_datetime_format(date_string: str) -> str:
    ...
    if "/" in date_string:
        if re.match(r"\d{1,2}/\d{1,2}/\d{4}$", date_string):
            ...
            month_fmt = (
                "%m" if len(parts[0]) == 2 else "%-m"
            )  # Use %-m for single-digit month
            day_fmt = (
                "%d" if len(parts[1]) == 2 else "%-d"
    ...
    replacements = [
        ...
        (
            r"[A-Za-z]{3,9} \d{2}, \d{4} \d{2}:\d{2}:\d{2}",
            "%B %d, %Y %H:%M:%S",
        ),  # "January 01, 2023 12:34:56"
        (
            r"[A-Za-z]{3,9} \d{1}, \d{4} \d{2}:\d{2}:\d{2}",
            "%B %-d, %Y %H:%M:%S",
