...
class TSDF:
    ...
    def __getLastRightRow(self, left_ts_col, right_cols, sequence_col, tsPartitionVal, ignoreNulls):
        ...
        window_spec = Window.partitionBy(self.partitionCols).orderBy(sort_keys).rowsBetween(Window.unboundedPreceding,
                                                                                            Window.currentRow)
        ...
        if ignoreNulls is False:
            ...
            df = reduce(
                lambda df, idx: df.withColumn(right_cols[idx], f.last(right_cols[idx], ignoreNulls).over(window_spec)),
                range(len(right_cols)), self.df)
        else:
            df = reduce(
                lambda df, idx: df.withColumn(right_cols[idx],
                                              f.last(right_cols[idx], ignoreNulls).over(window_spec)).withColumn(
        ...
        if tsPartitionVal is not None:
            for column in df.columns:
                if (column.startswith("non_null")):
                    ...
                    if any_blank_vals:
                        logger.warning(
                            "Column " + newCol + " had no values within the lookback window. Consider using a larger window to avoid missing values. If this is the first record in the data frame, this warning can be ignored.")
                    df = df.drop(column)
    ...
    def __getTimePartitions(self, tsPartitionVal, fraction=0.1):
        ...
        partition_df = (
            self.df
                .withColumn("ts_col_double",
                            f.col(self.ts_col).cast("double"))  # double is preferred over unix_timestamp
                .withColumn('ts_partition',
                            f.lit(tsPartitionVal) * (f.col("ts_col_double") / f.lit(tsPartitionVal)).cast('integer'))
                .withColumn("partition_remainder",
                            (f.col("ts_col_double") - f.col("ts_partition")) / f.lit(tsPartitionVal))
    ...
    def describe(self):
        ...
        missing_vals = this_df.select(
            [(100 * f.count(f.when(f.col(c[0]).isNull(), c[0])) / f.count(f.lit(1))).alias(c[0]) for c in this_df.dtypes
             if
        ...
        desc_stats = desc_stats.select(f.col("summary"), f.lit(" ").alias("unique_ts_count"),
                                       f.lit(" ").alias("min_ts"),
        ...
        global_smry_rec = desc_stats.limit(1).select(f.lit('global').alias("summary"),
                                                     f.lit(unique_ts).alias("unique_ts_count"),
                                                     f.lit(min_ts).alias("min_ts"), f.lit(max_ts).alias("max_ts"),
    ...
    def asofJoin(self, right_tsdf, left_prefix=None, right_prefix="right", tsPartitionVal=None, fraction=0.5,
                 skipNulls=True, sql_join_opt=False):
        """
    ...
    """
        ...
        if sql_join_opt & ((left_bytes < bytes_threshold) | (right_bytes < bytes_threshold)):
            ...
            quotes_df_w_lag = right_df.select(*new_right_cols).withColumn("lead_" + right_tsdf.ts_col,
                                                                          f.lead(right_prefix + right_tsdf.ts_col).over(
                                                                              w))
            left_df = left_df.select(*new_left_cols)
            res = left_df.join(quotes_df_w_lag, partition_cols).where(
                left_df[new_left_ts_col].between(f.col(right_prefix + right_tsdf.ts_col),
                                                 f.coalesce(f.col('lead_' + right_tsdf.ts_col),
                                                            f.lit('2099-01-01').cast("timestamp")))).drop(
        ...
        combined_df = (left_tsdf
                       ...
                       .__combineTSDF(right_tsdf.__addColumnsFromOtherDF(left_columns),
                                      combined_ts_col))
        combined_df.df = combined_df.df.withColumn("rec_ind",
                                                   f.when(f.col(left_tsdf.ts_col).isNotNull(), 1).otherwise(-1))

        # perform asof join.
        if tsPartitionVal is None:
            asofDF = combined_df.__getLastRightRow(left_tsdf.ts_col, right_columns, right_tsdf.sequence_col,
                                                   tsPartitionVal, skipNulls)
        else:
    ...
    def resample(self, freq, func=None, metricCols=None, prefix=None, fill=None):
        ...
        return (
            _ResampledTSDF(enriched_df, ts_col=self.ts_col, partition_cols=self.partitionCols, freq=freq, func=func))

    def interpolate(self, freq: str, func: str, method: str, target_cols: List[str] = None, ts_col: str = None,
                    partition_cols: List[str] = None, show_interpolated: bool = False):
        """
        ...
        interpolated_df: DataFrame = interpolate_service.interpolate(tsdf_input, ts_col, partition_cols, target_cols,
                                                                     freq, func, method, show_interpolated)
    ...
    def calc_bars(tsdf, freq, func=None, metricCols=None, fill=None):
        ...
        bars = resample_open.df.join(resample_high.df, join_cols).join(resample_low.df, join_cols).join(
            resample_close.df, join_cols)
