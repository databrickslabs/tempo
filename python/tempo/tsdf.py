...
class TSDF:
  ...
  def __validate_ts_string(self, ts_text):
      """Validate the format for the string using Regex matching for nanoseconds"""
      import re
  ...
  def withRangeStats(self, type='range', colsToSummarize=[], rangeBackWindowSecs=1000):
          ...
          # build window
          if ((str(self.df.schema[self.ts_col].dataType) == 'TimestampType') | (str(self.df.schema[self.ts_col].dataType) == 'StringType')):
