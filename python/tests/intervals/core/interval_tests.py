...
class TestInterval:
    ...
    def test_create_interval_series_fields_not_string(self):
        data = Series(
            {"start": None, "end": "2023-01-02", 1: "A", "metric1": 10, "metric2": 15}
        )

        with pytest.raises(
            InvalidSeriesColumnError, match="All series_fields must be strings"
        ):
            Interval.create(
                data,
                start_field="start",
    ...
    def test_create_interval_series_fields_not_sequence(self):
        ...
        with pytest.raises(
            InvalidSeriesColumnError, match=r"series_fields must be a sequence"
        ):
            Interval.create(
                data, start_field="start", end_field="end", series_fields="series-id1"
            )

    def test_create_interval_metrics_fields_not_string(self):
        data = Series(
            {"start": None, "end": "2023-01-02", "series": "A", 1: 10, "metric": 15}
        )

        with pytest.raises(
            InvalidMetricColumnError, match="All metric_fields must be strings"
        ):
            Interval.create(
                data,
                start_field="start",
    ...
    def test_create_interval_metrics_fields_not_sequence(self):
        data = Series(
            {"start": None, "end": "2023-01-02", "series": "A", 1: 10, "metric2": 15}
        )

        with pytest.raises(
            InvalidMetricColumnError, match="metric_fields must be a sequence"
        ):
            Interval.create(
                data,
                start_field="start",
    ...
    def test_validate_metric_alignment(self):
        ...
        data2 = Series(
            {"start": "2023-01-02", "end": "2023-01-04", "metric1": 5, "metric2": 10}
        )
    ...
    def test_validate_metric_alignment_invalid(self):
        ...
        interval2 = Interval.create(
            data2, start_field="start", end_field="end", metric_fields=["metric1"]
        )

        expected_msg = re.escape(
            "metric_fields don't match: ['metric1', 'metric2'] vs ['metric1']"
        )
    ...
    def test_validate_series_alignment_invalid(self):
        ...
        expected_msg = re.escape(
            "series_fields don't match: ['series1'] vs ['series1', 'series2']"
        )
    ...
    def test_validate_not_point_in_time_point_in_time_interval(self):
        ...
        with pytest.raises(
            InvalidDataTypeError, match="Point-in-Time Intervals are not supported"
        ):
            Interval._validate_not_point_in_time(data, boundary_accessor)
...
class TestStillValidLegacy:
    ...
    def test_update_interval_boundary_start(self):
        interval = Interval.create(
            Series({"start": "2023-01-01T01:00:00", "end": "2023-01-01T02:00:00"}),
            "start",
            "end",
        )
    ...
    def test_merge_metrics_with_string_metric_columns(self):
        interval = Interval.create(
            Series({"start": "01:00", "end": "02:00", "value1": 10, "value2": 20}),
            "start",
            "end",
            metric_fields=["value1", "value2"],
        )
        other = Interval.create(
            Series({"start": "01:00", "end": "02:00", "value1": 20, "value2": 30}),
            "start",
            "end",
            metric_fields=["value1", "value2"],
        )
