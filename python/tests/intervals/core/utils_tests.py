...
class TestIntervalUtils:
    ...
    def test_init(self, intervals_utils, interval_data):
        ...
        pd.testing.assert_frame_equal(
            intervals_utils.intervals, interval_data["intervals_data"]
        )
    ...
    def test_calculate_all_overlaps(
        self, intervals_utils, reference_interval, interval_data
    ):
    ...
    def test_find_overlaps(self, intervals_utils, reference_interval, interval_data):
        ...
        intervals_with_reference.loc[len(intervals_with_reference)] = interval_data[
            "reference_interval_data"
        ]
    ...
    def test_add_as_disjoint_empty_disjoint_set(
        self, intervals_utils, reference_interval, interval_data
    ):
    ...
    def test_add_as_disjoint_no_overlaps(
        self, mock_transformer, intervals_utils, reference_interval, interval_data
    ):
    ...
    @patch("tempo.intervals.core.utils.IntervalTransformer")
    def test_add_as_disjoint_with_duplicate(
        self, mock_transformer, intervals_utils, reference_interval, interval_data
    ):
        """Test add_as_disjoint with duplicate interval."""
        # Set up disjoint set with the reference interval already in it
        intervals_utils.disjoint_set = DataFrame(
            [interval_data["reference_interval_data"]]
        )
    ...
    def test_add_as_disjoint_single_overlap(
        self, mock_resolve_overlap, intervals_utils, reference_interval, interval_data
    ):
    ...
    def test_add_as_disjoint_multiple_overlaps(
        self,
        mock_resolve_all_overlaps,
        intervals_utils,
        reference_interval,
        interval_data,
    ):
    ...
    @patch("tempo.intervals.core.utils.IntervalTransformer")
    def test_add_as_disjoint_mixed_overlaps(
        self, mock_transformer, intervals_utils, reference_interval, interval_data
    ):
    ...
    def test_calculate_all_overlaps_touching_intervals(
        self, intervals_utils, interval_data
    ):
        ...
        tz_intervals = DataFrame(
            {
                ...
                end_field: [
                    pd.to_datetime("2023-01-07").tz_localize("UTC"),
...
class TestResolveAllOverlaps:
    ...
    def test_resolve_all_overlaps(
        ...
        with patch.object(IntervalsUtils, "find_overlaps") as mock_find_overlaps:
            ...
            with patch.object(
                IntervalsUtils, "add_as_disjoint"
            ) as mock_add_as_disjoint:
    ...
    def test_resolve_all_overlaps_with_single_item(
        self, intervals_utils, reference_interval, interval_data
    ):
        ...
        with patch(
            "tempo.intervals.overlap.transformer.IntervalTransformer.resolve_overlap"
        ) as mock_resolve:
    ...
    def test_resolve_all_overlaps_with_complex_overlaps(self, interval_data):
        ...
        reference_interval = Interval.create(
            reference_data, start_field, end_field, series_fields, metric_fields
        )
        ...
        with patch(
            "tempo.intervals.overlap.transformer.IntervalTransformer.resolve_overlap",
            side_effect=resolve_overlap_wrapper,
        ) as mock_resolve:
                    ...
                    DataFrame(
                        {
                            ...
                            end_field: pd.to_datetime(
                                ["2023-01-02", "2023-01-06", "2023-01-07"]
                            ),
                            "category": ["A", "X", "X"],
                            "value": [10, 50, 50],
                        }
    ...
    def test_resolve_all_overlaps_with_partially_overlapping_intervals(
        self, interval_data
    ):
    ...
    def test_resolve_all_overlaps_with_completely_contained_intervals(
        self, interval_data
    ):
    ...
    def test_resolve_all_overlaps_with_identical_intervals(self, interval_data):
        ...
        reference_interval = Interval.create(
            identical_data.copy(), start_field, end_field, series_fields, metric_fields
        )
    ...
    def test_resolve_all_overlaps_recursive_call_structure(self, interval_data):
        ...
        with patch(
            "tempo.intervals.overlap.transformer.IntervalTransformer"
            ...
            for i in range(3):
                ...
                mock_transformer_cls(
                    interval=reference_interval, other=reference_interval
                )
    ...
    def test_resolve_all_overlaps_with_mock_and_functionality(self, interval_data):
        ...
        with patch(
            "tempo.intervals.core.utils.IntervalTransformer"
        ) as mock_transformer_cls:
            ...
            with patch.object(
                IntervalsUtils, "_calculate_all_overlaps"
            ) as mock_calc_overlaps:
    ...
    def test_resolve_all_overlaps_with_no_overlaps(self, interval_data):
        ...
        with patch.object(
            IntervalsUtils, "_calculate_all_overlaps", return_value=DataFrame()
        ):
            result = utils.resolve_all_overlaps(reference_interval)
...
class TestAddAsDisjoint:
    def test_add_as_disjoint_no_overlap(self, interval_data):
        ...
        new_interval = Interval.create(
            new_data, start_field, end_field, series_fields, metric_fields
        )
    ...
    def test_add_as_disjoint_with_overlap(self, interval_data):
        ...
        new_data[start_field] = pd.to_datetime(
            "2023-01-12"
        )  # Use datetime objects to match
        ...
        with patch.object(IntervalsUtils, "find_overlaps") as mock_find_overlaps:
            ...
            with patch(
                ...
                for (
                    series_data
                ) in mock_transformer_instance.resolve_overlap.return_value:
                ...
                expected_df = (
                    ...
                    .sort_values(by=start_field)
                    .reset_index(drop=True)
                )

                # Sort the result by start time as well
                sorted_result = result.sort_values(by=start_field).reset_index(
                    drop=True
                )
    ...
    def test_add_as_disjoint_duplicate(self, interval_data):
        ...
        duplicate_interval = Interval.create(
            duplicate_data, start_field, end_field, series_fields, metric_fields
        )
    ...
    def test_add_as_disjoint_empty_disjoint_set(self, interval_data):
        ...
        new_data = Series({start_field: 4, end_field: 8, "metric": 30})
    ...
    def test_add_as_disjoint_multiple_to_resolve_not_only_overlaps(
        self, intervals_utils, reference_interval, interval_data
    ):
        ...
        with patch.object(IntervalsUtils, "find_overlaps", return_value=overlapping_df):
            ...
            with patch(
                "tempo.intervals.core.utils.IntervalsUtils.resolve_all_overlaps",
                ...
                expected_df = pd.concat([resolved_df, non_overlapping_df])[
                    expected_cols
                ]
    ...
    def test_add_as_disjoint_multiple_resolve_not_only_overlaps_corner_case(
        self, interval_data
    ):
        ...
        with patch.object(
            IntervalsUtils, "find_overlaps", return_value=overlapping_subset_df
        ):
    ...
    def test_add_as_disjoint_unexpected_conditions_raises_error(self, interval_data):
        ...
        try:
            ...
            with pytest.raises(
                NotImplementedError, match="Interval resolution not implemented"
            ):
...
class TestStillValidLegacy:
    def test_identify_interval_overlaps_df_empty(self):
        df = pd.DataFrame()
        row = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:01", "end": "2023-01-01T00:00:05"}),
            "start",
            "end",
        )
    ...
    def test_identify_interval_overlaps_overlapping_intervals(self):
        ...
        row = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:03", "end": "2023-01-01T00:00:06"}),
            "start",
            "end",
        )
    ...
    def test_identify_interval_overlaps_no_overlapping_intervals(self):
        ...
        row = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:04.1", "end": "2023-01-01T00:00:05"}),
            "start",
            "end",
        )
    ...
    def test_identify_interval_overlaps_identical_start_end(self):
        ...
        row = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:02", "end": "2023-01-01T00:00:05"}),
            "start",
            "end",
        )
    ...
    def test_add_as_disjoint_where_duplicate_interval(self):
        ...
        pd.testing.assert_frame_equal(
            result.reset_index(drop=True), disjoint_set.reset_index(drop=True)
        )
