...
class TestIntervalTransformerInit:
    ...
    def test_init_orders_intervals_correctly(self, intervals):
        ...
        earlier_interval = Interval.create(
            earlier_data, "start", "end", metric_fields=["metric1", "metric2"]
        )
        ...
        equal_start_interval = Interval.create(
            equal_start_data, "start", "end", metric_fields=["metric1", "metric2"]
        )
    ...
    def test_validate_intervals(self, intervals):
        ...
        assert "Expected indices of interval elements to be equivalent" in str(
            excinfo.value
        )
...
class TestIntervalTransformerRelationships:
    ...
    def test_detect_relationship(
        self, interval_pairs, relationship_name, expected_type, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            checker_map[expected_type], "check", lambda self, a, b: True
        )
        ...
        with patch.object(
            transformer,
            "detect_relationship",
            return_value=OverlapResult(type=expected_type),
        ):
        ...
        with patch.object(MetricsEquivalentChecker, "check", return_value=True):
            ...
            with patch.object(
                ...
                return_value=OverlapResult(
                    type=OverlapType.METRICS_EQUIVALENT, details={"metrics_match": True}
                ),
            ):
                result = transformer.detect_relationship()
                assert isinstance(result, OverlapResult)
    ...
    def test_resolve_overlap(self, intervals, relationship_type):
        ...
        with (
            patch.object(
                transformer, "detect_relationship", return_value=relationship_type
            ),
            ...
            mock_resolver.resolve.assert_called_once_with(
                transformer.interval, transformer.other
            )
        ...
        with (
            patch.object(
                transformer, "detect_relationship", return_value=overlap_result
            ),
            patch.object(transformer, "_get_resolver", return_value=mock_resolver),
        ):
    ...
    def test_resolve_overlap_no_relationship(self, intervals):
        ...
        with patch.object(transformer, "detect_relationship", return_value=None):
            with pytest.raises(
                NotImplementedError, match="Unable to determine interval relationship"
            ):
        ...
        with patch.object(
            transformer, "detect_relationship", return_value=OverlapResult(type=None)
        ):
            with patch.object(
                transformer, "resolve_overlap", side_effect=mock_resolve_overlap
            ):
                with pytest.raises(
                    NotImplementedError,
                    match="Unable to determine interval relationship",
                ):
                    transformer.resolve_overlap()
    ...
    def test_no_resolver_found_during_resolve(self, intervals):
        ...
        with patch.object(
            transformer, "detect_relationship", return_value=unknown_type
        ):
...
class TestIntervalTransformerIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "during",
                3,
            ),  # Before contained, contained with merged metrics, after contained
    ...
    def test_interval_resolution(
        self, interval_pairs, relationship_name, expected_segments
    ):
        ...
        result_series = [
            pd.Series({"metric1": 10, "metric2": 20}) for _ in range(expected_segments)
        ]
        ...
        with patch.object(
            ...
            with patch.object(
                transformer, "resolve_overlap", return_value=result_series
            ) as mock_resolve:
                result = transformer.resolve_overlap()
...
class TestPrecisionEdgeCases:
    ...
    def test_meets_exact_timestamp(self):
        ...
        assert (
            relationship == OverlapType.MEETS
        ), f"Expected MEETS relationship, got {relationship}"
    ...
    def test_meets_floating_point_precision(self):
        ...
        interval1 = Interval.create(
            data=data1, start_field="start", end_field="end", metric_fields=["metric1"]
        )

        interval2 = Interval.create(
            data=data2, start_field="start", end_field="end", metric_fields=["metric1"]
        )
        ...
        ), "Should detect a relationship despite precision differences"
...
class TestNullMetricValues:
    ...
    def test_nan_metric_values(self):
        ...
        assert (
            relationship == OverlapType.OVERLAPS
        ), f"Expected OVERLAPS relationship, got {relationship}"
        ...
        print(
            f"NaN handling in overlapping segment: metric2 = {middle_segment['metric2']}"
        )
...
class TestActualResolverImplementations:
    ...
    def test_overlaps_resolver(self, overlapping_intervals):
        ...
        print(
            f"Metrics in overlapping segment: metric1={resolved[1]['metric1']}, metric2={resolved[1]['metric2']}"
        )
    ...
    def test_equals_resolver(self, equal_intervals):
        ...
        # Verify relationship is as expected
        assert (
            relationship == OverlapType.EQUALS
        ), f"Expected EQUALS relationship, got {relationship}"
        ...
        # Print actual values for reference
        print(
            f"Metrics in equals result: metric1={resolved[0]['metric1']}, metric2={resolved[0]['metric2']}"
        )
    ...
    def test_contains_resolver(self, containing_intervals):
        ...
        # Verify relationship is as expected
        assert (
            relationship == OverlapType.CONTAINS
        ), f"Expected CONTAINS relationship, got {relationship}"
        ...
        # Print actual values for reference
        print(
            f"Metrics in contained segment: metric1={resolved[1]['metric1']}, metric2={resolved[1]['metric2']}"
        )
