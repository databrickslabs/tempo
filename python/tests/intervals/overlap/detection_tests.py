        ...
        for i, interval_a in enumerate(intervals):
            for j, interval_b in enumerate(intervals):
                ...
                for relation_a_to_b in true_relations_a_to_b:
                    ...
                    assert checkers[inverse_relation].check(
                        interval_b, interval_a
                ...
                if len(true_relations_a_to_b) == 1:
                    ...
                    assert (
                        true_relations_b_to_a[0] == inverse_relation
...
class TestBoundaryEdgeCases:
    ...
    def test_zero_length_intervals_special_cases(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_adjacent_intervals_combinations(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_nested_intervals_shared_boundaries(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_complex_chain_of_relations(self, mock_values, checkers, create_interval):
        ...
        interval_4 = create_interval(
            v3, v4
        )  # [v3,v4] - during interval_3, after interval_2
        interval_5 = create_interval(
            v4, v5
        )  # [v4,v5] - meets interval_3, meets interval_4
...
class TestSpecialBoundaryScenarios:
    ...
    def test_intervals_with_multi_boundary_sharing(
        self, mock_values, checkers, create_interval
    ):
        ...
        for i in range(len(common_start)):
            for j in range(i + 1, len(common_start)):
                if i == j:
                    continue
                assert checkers["starts"].check(
                    common_start[i], common_start[j]
                ) or checkers["started_by"].check(common_start[i], common_start[j])
        ...
        for i in range(len(common_end)):
            for j in range(i + 1, len(common_end)):
                if i == j:
                    continue
                assert checkers["finishes"].check(
                    common_end[i], common_end[j]
                ) or checkers["finished_by"].check(common_end[i], common_end[j])


class TestAdvancedScenarios:
    """Tests for edge cases and combined usage scenarios"""

    def test_multiple_checkers_combination(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_edge_case_zero_length_intervals_before_after(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_all_relations_are_mutually_exclusive(
        self, mock_values, checkers, create_interval
    ):
        ...
        relation_checkers = [
            ...
            checkers["contains"],
            checkers["started_by"],
            checkers["finished_by"],
            checkers["overlapped_by"],
            checkers["met_by"],
            checkers["after"],
        ]
    ...
    def test_inverse_relations(self, mock_values, checkers, create_interval):
        ...
        assert (
            checkers["finishes"].check(interval_a, interval_e) is False
        )  # Not true for this example
        assert (
            checkers["finished_by"].check(interval_e, interval_a) is False
        )  # Should match above
    ...
    def test_mutual_exclusivity_regular_intervals(
        self, mock_values, checkers, create_interval
    ):
        ...
        intervals = {
            ...
            "finished_by": create_interval(
                v1, v4
            ),  # Contains and shares end with reference
            "contains": create_interval(v1, v5),  # Strictly contains reference
            "started_by": create_interval(
                v2, v5
            ),  # Shares start and extends beyond reference
            "overlapped_by": create_interval(
                v3, v5
            ),  # Reference overlaps start of this interval
...
class TestMetricsEdgeCases:
    ...
    def test_case_sensitivity_in_metric_names(
        self, mock_values, checkers, create_interval
    ):
    ...
    def test_special_characters_in_metrics(
        self, mock_values, checkers, create_interval
    ):
...
class TestPracticalScenarios:
    ...
    def test_calendar_event_overlaps(self, mock_values, checkers, create_interval):
        ...
        assert (
            checkers["meets"].check(meeting_10am_11am, lunch_12pm_1pm) is True
        )  # Fix for original assertion

        # Test 'before' relationship (no shared endpoints)
        assert (
            checkers["before"].check(meeting_9am_10am, lunch_12pm_1pm) is True
        )  # 9am-10am is before 12pm-1pm
        assert (
            checkers["before"].check(meeting_10am_11am, afternoon_meeting) is True
        )  # 10am-11am is before 1pm-2pm
    ...
    def test_process_monitoring_intervals(self, mock_values, checkers, create_interval):
        ...
        assert (
            checkers["metrics_equivalent"].check(system_uptime, maintenance_window)
            is True
        )
        ...
        assert (
            checkers["metrics_equivalent"].check(system_uptime, other_system) is False
        )
...
class TestMutualExclusivity:
    ...
    def test_comprehensive_mutual_exclusivity(
        self, mock_values, checkers, create_interval
    ):
        """
        ...
        for name_a, interval_a in intervals.items():
            for name_b, interval_b in intervals.items():
                ...
                if relation_a_to_b:
                    ...
                    assert checkers[expected_relation_b_to_a].check(
                        interval_b, interval_a
    ...
    def test_identity_relations(self, mock_values, checkers, create_interval):
        ...
        for interval in test_intervals:
            ...
            for relation in non_equals_relations:
                assert not checkers[relation].check(
                    interval, interval
    ...
    def test_transitivity_properties(self, mock_values, checkers, create_interval):
        ...
        assert checkers["contains"].check(
            a_contains, c_contains
        ), "Contains relation should be transitive"

    def test_exhaustive_pairwise_relations(
        self, mock_values, checkers, create_interval
    ):
        """
        ...
        for start_idx in range(1, 5):  # Using 1-4 as start indices
            for end_idx in range(start_idx + 1, 6):  # Using start+1 to 5 as end indices
                ...
                intervals.append(
                    (f"v{start_idx}_v{end_idx}", create_interval(start_val, end_val))
                )
    ...
    def test_shared_endpoints_exhaustive(self, mock_values, checkers, create_interval):
        ...
        for (
            ...
            relation_a_to_b,
            relation_b_to_a,
        ) in shared_endpoint_pairs:
            # Test forward relation
            assert checkers[relation_a_to_b].check(
                interval_a, interval_b
            ), f"Expected {relation_a_to_b} to be true from A to B"

            # Test inverse relation
            assert checkers[relation_b_to_a].check(
                interval_b, interval_a
...
class TestTransitivityProperties:
    ...
    def test_before_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["before"].check(
            interval_d, interval_f
        ), "Before relation should be transitive for non-point intervals"
    ...
    def test_equals_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["equals"].check(
            interval_a, interval_c
        ), "Equals relation should be transitive"
    ...
    def test_starts_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["starts"].check(
            interval_a, interval_c
        ), "The 'starts' relation is transitive"
    ...
    def test_finishes_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["finishes"].check(
            interval_a, interval_c
        ), "The 'finishes' relation is transitive"
    ...
    def test_meets_non_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["before"].check(
            interval_a, interval_c
        ), "When A meets B and B meets C, A is before C"
    ...
    def test_overlaps_non_transitivity(self, mock_values, checkers, create_interval):
        ...
        assert checkers["meets"].check(
            interval_a, interval_c
        ), "When A overlaps B and B overlaps C, A may meet C"
...
class TestStillValidLegacy:
    def test_interval_starts_with_other_shorter_duration(self):
        ...
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:00", "end": "2023-01-01T00:00:01"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:00", "end": "2023-01-01T00:00:02"}),
            "start",
            "end",
        )
    ...
    def test_interval_is_started_by_other(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:01", "end": "2023-01-01T00:00:03"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:01", "end": "2023-01-01T00:00:02"}),
            "start",
            "end",
        )
        ...
        assert not StartsChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
        assert not EqualsChecker().check(interval, other)
    ...
    def test_interval_contains_other(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:00", "end": "2023-01-01T03:00:00"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T01:00:00", "end": "2023-01-01T02:00:00"}),
            "start",
            "end",
        )
        ...
        assert not DuringChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
        assert not EqualsChecker().check(interval, other)
    ...
    def test_interval_overlaps_but_not_contained(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:00", "end": "2023-01-01T01:30:00"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T01:00:00", "end": "2023-01-01T03:00:00"}),
            "start",
            "end",
        )
        ...
        assert not ContainsChecker().check(interval, other)
        assert not StartsChecker().check(interval, other)
        assert not OverlappedByChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
    ...
    def test_interval_is_overlapped_by_other(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T02:00:00", "end": "2023-01-01T05:00:00"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T01:00:00", "end": "2023-01-01T04:00:00"}),
            "start",
            "end",
        )
        ...
        assert not DuringChecker().check(interval, other)  # Verify not contained
        assert not OverlapsChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
        assert not ContainsChecker().check(interval, other)
    ...
    def test_interval_before_other_no_overlap(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:00", "end": "2023-01-01T01:00:00"}),
            "start",
            "end",
        )
        other = Interval.create(
            pd.Series({"start": "2023-01-01T02:00:00", "end": "2023-01-01T03:00:00"}),
            "start",
            "end",
        )
        ...
        assert not DuringChecker().check(interval, other)  # Verify not contained
        assert not MeetsChecker().check(
            interval, other
        )  # Verify no touching boundaries
        assert not OverlapsChecker().check(interval, other)  # Verify no overlap
        assert not AfterChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
    ...
    def test_intervals_are_equal(self):
        ...
        # Arrange
        interval = Interval.create(
            pd.Series({"start": "2023-01-01T01:00:00", "end": "2023-01-01T01:30:00"}),
            "start",
            "end",
        )
        other = Interval.create(
    ...
    def test_interval_overlaps_other(self):
        ...
        other = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:02", "end": "2023-01-01T00:00:04"}),
            "start",
            "end",
        )
    ...
    def test_interval_starts_with_other(self):
        ...
        other = Interval.create(
            pd.Series({"start": "2023-01-01T00:00:01", "end": "2023-01-01T00:00:04"}),
            "start",
            "end",
        )
        ...
        assert not DuringChecker().check(interval, other)
        assert not OverlapsChecker().check(interval, other)
        assert not StartedByChecker().check(
            interval, other
        )  # Important! This is the inverse relationship
